    ...
    <section class="sectionlist show" id="formarea">
      <cfif cgi.request_method Eq 'post'>
        ...
        <cftry>
          ...
          <cfif #StructKeyExists(form,'areaofinterest')# eq false>
            <cfset #areaofinterestvar# = 'None'/>
            <cfelse>
            <cfset #areaofinterestvar# = #form.areaofinterest#/>
            <cfif #FindNoCase("others",form.areaofinterest)# lte 0 >
              <cfset session.other_interest = "" />
              <cfquery name="UpdateStudentProfile" datasource="cccac_swipe">
                  update ccac_registered_users
                    set
                    email = '#form.email#',phone_number = '#form.phone#', major = '#form.major#', year_in_college = '#form.year#', area_of_interest = '#areaofinterestvar#',area_of_interest_value ='', last_update_date = #myDateTime#,last_update_mode = '#session.signinmethod#', subscribed_to_newsletter=#newsletter#
                    where
              ...
              <cfquery name="UpdateStudentProfile" datasource="cccac_swipe">
                    update ccac_registered_users
                      set
                      email = '#form.email#',phone_number = '#form.phone#', major = '#form.major#', year_in_college = '#form.year#', area_of_interest = '#areaofinterestvar#', area_of_interest_value ='#form.otherinterest#', last_update_date = #myDateTime#,last_update_mode = '#session.signinmethod#', subscribed_to_newsletter=#newsletter#
                      where
                      student_id = #session.studentid#
                    </cfquery>
            </cfif>
