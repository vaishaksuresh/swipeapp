...
<section id="pages" class="group">
  <div id="loadcontent" class="group">
    <section class="sectionlist show" id="formarea">
      ...
      <cfform style="padding-left:15px;">
        ...
        <cfinput type="button" class="blue_button" name="Download" value="Download" onClick="doDownload()">
        </br></br>
  ...
  <cfajaxproxy bind="javascript:noteChange({FirstGrid.name},{FirstGrid.email},{FirstGrid.last_login_date},{FirstGrid.last_update_date},{FirstGrid.last_update_mode},{FirstGrid.NewsLetter})">
  <script>
	function noteChange(name,email,lastlogin,lastupdate,lastupdatemode,subscription) {
		$('#details').html("<p>Name: "+name+"<br>Email: "+email+"<br>Last Login: "+lastlogin+"<br>Last Update: "+lastupdate+","+lastupdatemode+"<br>Newsletter Subscription: "+subscription+"</p>");
	}
</script>
